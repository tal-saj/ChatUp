...
export default function Chat() {
  ...
  const handleStartCall = useCallback(
    async ({ contact, callType = "audio" }) => {
      if (callActive) return; // Edge case: already in a call
      ...
      try {
        ...
        if (err?.name === "NotAllowedError") {
          alert(
            callType === "video"
              ? "Camera/microphone access denied. Please allow it in browser settings."
  ...
  const handleAcceptCall = useCallback(
    async (incomingCall) => {
      // Edge case: already in a call (e.g. another tab or race condition)
      if (callActive) {
        // Silently reject the second call
        try {
      ...
      try {
        ...
        setCallActive(true);
        setCallerStatus(null); // callee — no ringing status
  ...
  // ── Video toggle (from ActiveCall controls) ───────────────────────────────────
  const handleVideoChange = useCallback(
    (isOff) => {
      setVideoEnabled(!isOff); // isOff=true → enabled=false
    },
    [setVideoEnabled]
  );
  ...
  useEffect(() => {
    ...
    const fetchUnread = async () => {
      try {
        const { data } = await api.post(unreadCountsRoute, { since: unreadSince });
  ...
  }, [currentUser, unreadSince]);

  // ── 6. Inactivity logout (30 min) ────────────────────────────────────────────
  useEffect(() => {
    if (!currentUser) return;
    let timer;
    const reset = () => {
  ...
  return (
    <div className={`relative flex h-screen w-screen overflow-hidden transition-colors duration-300 ${
      ...
      {/* ── Incoming call banner — hidden while in a call ── */}
      {!callActive && (
        <IncomingCallBanner
      ...
      {callActive && activeCall && (
        <ActiveCall
          ...
          localStreamGetter={getLocalStream}
            ...
            `}>
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className={`text-xs ${
                    currentChat.online ? "text-emerald-500" : darkMode ? "text-slate-500" : "text-slate-400"
                  }`}>
