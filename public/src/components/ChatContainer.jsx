...
export default function ChatContainer({
  ...
  return (
    // KEY FIX: h-full ensures this fills whatever parent gives it.
    // flex flex-col means header + messages + input stack vertically.
    // overflow-hidden prevents the whole thing from scrolling — only <main> scrolls.
    <div className={`flex h-full flex-col overflow-hidden transition-colors duration-300 ${
      dm
        ? "bg-[#0f172a]"
      ...
      {/* Header */}
      <header className={`
        flex-shrink-0 px-4 py-3.5 flex items-center justify-between border-b transition-colors duration-300
        ${dm
          ...
          <div className="relative">
            <div className={`h-10 w-10 rounded-full overflow-hidden ring-2 ring-offset-2 flex-shrink-0 ${
              ...
              <img
                ...
                alt={currentChat?.username ?? "Contact"}
      ...
      {/* E2E info banner */}
      {!isLoading && (
        <div className={`flex-shrink-0 mx-4 mt-3 mb-1 flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs ${
          dm
      ...
      {/* Messages — flex-1 + overflow-y-auto = this section scrolls, nothing else does */}
      ...
      {/* Input — flex-shrink-0 keeps it pinned to bottom, safe-area padding for iPhones */}
      <div
        className={`flex-shrink-0 px-3 pt-2 border-t transition-colors duration-300 ${
          dm
