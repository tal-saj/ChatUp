...
export default function Contacts({ contacts, changeChat, darkMode, toggleDarkMode }) {
  ...
  return (
    <div className={`flex h-full flex-col transition-colors duration-300 ${darkMode ? "bg-slate-900" : "bg-gradient-to-b from-slate-50 via-slate-100 to-white"}`}>
      ...
      <div className={`shrink-0 flex items-center gap-3 py-4 px-4 border-b transition-colors duration-300 ${
        ...
        <h3 className={`text-lg font-bold tracking-tight flex-1 ${darkMode ? "text-slate-100" : "text-slate-800"}`}>
          ChatUp
        </h3>
        ...
        {contacts.map((contact, index) => {
          ...
          return (
            <div
              ...
              className={`
                group flex items-center gap-3.5 px-4 py-3 rounded-xl cursor-pointer
                transition-colors duration-150
                ${isSelected
                  ? darkMode
                    ? "bg-slate-700 border border-slate-600/80 shadow-md"
                    : "bg-white border border-slate-300/80 shadow-md shadow-slate-400/20"
                  : darkMode
            ...
            >
              ...
              {isSelected && (
                <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${darkMode ? "bg-indigo-400" : "bg-slate-600"}`} />
              )}
            </div>
          );
        })}
      ...
      <div className={`shrink-0 border-t px-4 py-3.5 flex items-center gap-3 transition-colors duration-300 ${
        ...
        <div className="min-w-0 flex-1">
          <h2 className={`font-semibold truncate text-sm ${darkMode ? "text-slate-100" : "text-slate-900"}`}>
            {currentUserName}
          </h2>
          <p className={`text-xs font-medium ${darkMode ? "text-slate-500" : "text-slate-400"}`}>Online</p>
        </div>
